...
static void vendor_model_srv_rsp_handler(const vendor_model_srv_status_t *val) {
    ...
    if (val->vendor_model_srv_Hdr.opcode == OP_VENDOR_MESSAGE_TRANSPARENT_MSG) {
        ...
        APP_DBG("从0x%04x收到数据%s,长度为%d", val->vendor_model_srv_Event.trans.addr, recv,
                val->vendor_model_srv_Event.trans.len);
        HandleReceivedData(val->vendor_model_srv_Event.trans.addr, val->vendor_model_srv_Event.trans.group_addr, recv,
                           val->vendor_model_srv_Event.trans.len);
        ...
        APP_DBG("从0x%04x收到write数据%s,长度为%d", val->vendor_model_srv_Event.write.addr, recv,
...
void App_Init() {
    ...
    InitDataTransfer(RecvMessage, ErrorHandler);
}
